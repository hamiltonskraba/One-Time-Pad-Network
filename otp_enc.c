...
int main(int argc, char* argv[]){
	...
	//write plaintext data to server---------------------------------------------------------
	...
	memset(recBuff, '\0', sizeof(recBuff));						//clear buffer

	int testRead = recv(socketFD, recBuff, sizeof(recBuff), 0);			//read in confirmation message
	
	if(testRead < 0){
		fprintf(stderr, "Error: Unable to read from server socket\n");			//check for read error
		exit(2);
	}

	//printf("Client recieved from server: %s\n", recBuff);

	if(strcmp(recBuff, "encserv?") != 0){						//confirm that it's connected to enc, not dec
		fprintf(stderr, "Error: mismatch in ports\n");
		exit(2);
	}

	//write key data to server--------------------------------------------------------------
	dataSent = send(socketFD, keyBuff, strlen(keyBuff), 0);			
	...
	//read back encrypted data from server-------------------------------------------------
	memset(recBuff, '\0', BUFFSIZE);						//clear the buffer

	int cipherRead = recv(socketFD, recBuff, BUFFSIZE, 0);				
